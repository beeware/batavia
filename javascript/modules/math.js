...
export var math = {
    ...
    'nan': pyfloat(NaN),
    'pi': pyfloat(Math.PI),
    'inf': pyfloat(Infinity)
}

var _checkfloat = function(x) {
    if (isinstance(x, pycomplex)) {
        throw pyTypeError("can't convert complex to float")
...
math.acos = function(x) {
    _checkfloat(x)
    return pyfloat(Math.acos(x.__float__().$val))
}
...
math.asin = function(x) {
    _checkfloat(x)
    return pyfloat(Math.asin(x.__float__().$val))
}
math.asin.__doc__ = 'asin(x)\n\nReturn the arc sine (measured in radians) of x.'

math.asinh = function(x) {
    _checkfloat(x)
    return pyfloat(Math.asinh(x.__float__().$val))
}
math.asinh.__doc__ = 'asinh(x)\n\nReturn the inverse hyperbolic sine of x.'

math.atan = function(x) {
    _checkfloat(x)
    return pyfloat(Math.atan(x.__float__().$val))
}
...
math.atan2 = function(y, x) {
    ...
    return pyfloat(Math.atan2(yy, xx))
}
...
math.atanh = function(x) {
    ...
    return pyfloat(Math.atanh(x.__float__().$val))
}
...
math.ceil = function(x) {
    ...
    _checkfloat(x)
    return pyint(Math.ceil(x.__float__().$val))
}
...
math.cos = function(x) {
    _checkfloat(x)
    return pyfloat(Math.cos(x.__float__().$val))
}
...
math.cosh = function(x) {
    ...
    if (!isFinite(result)) {
        throw pyOverflowError('math range error')
    }
    return pyfloat(Math.cosh(x.__float__().$val))
}
...
math.degrees = function(x) {
    ...
    return pyfloat(x.__float__().$val * 57.295779513082322865)
}
...
math.erf = function(x) {
    ...
    return pyfloat(sign * result)
}
math.erf.__doc__ = 'erf(x)\n\nError function at x.'

math.erfc = function(x) {
    _checkfloat(x)
    return pyfloat(1.0 - math.erf(x).$val)
}
...
math.expm1 = function(x) {
    ...
    return pyfloat(Math.expm1(x.__float__().$val))
}
math.expm1.__doc__ = 'expm1(x)\n\nReturn exp(x)-1.\nThis function avoids the loss of precision involved in the direct evaluation of exp(x)-1 for small x.'

math.fabs = function(x) {
    _checkfloat(x)
    return pyfloat(Math.abs(x.__float__().$val))
}
...
math.factorial = function(x) {
    ...
    if (isinstance(x, pyint)) {
        ...
        if (!x.is_integer().valueOf()) {
            throw pyValueError('factorial() only accepts integral values')
        }
        ...
        throw pyTypeError('an integer is required (got type pyNoneType)')
    } else {
        throw pyTypeError('an integer is required (got type ' + x.__class__.__name__ + ')')
    }

    if (num.isNegative()) {
        throw pyValueError('factorial() not defined for negative values')
    }
    ...
    return pyint(_mul_list(nums, 0, nums.length - 1))
}
...
math.floor = function(x) {
    ...
    _checkfloat(x)
    return pyint(Math.floor(x.__float__().$val))
}
...
math.frexp = function(x) {
    ...
    if (xx === 0 || !isFinite(xx)) {
        return pytuple([x.__float__(), pyint(0)])
    }
    ...
    return pytuple([pyfloat(num), pyint(exp)])
}
...
math.gamma = function(x) {
    ...
    if (xx <= 0.0) {
        ...
        return pyfloat(Math.PI / Math.sin(Math.PI * xx) / math.gamma(pyfloat(1 - xx)))
    }
    ...
    if (xx < 0.001) {
        return pyfloat(1.0 / (x * (1.0 + gamma * x)))
    }
...
math.gcd = function(x, y) {
    if (!isinstance(x, [pybool, pyint])) {
        throw pyTypeError("'" + type_name(x) + "' object cannot be interpreted as an integer")
    }
    if (!isinstance(y, [pybool, pyint])) {
        throw pyTypeError("'" + type_name(y) + "' object cannot be interpreted as an integer")
    }
...
math.hypot = function(x, y) {
    ...
    _checkfloat(x)
    var xx = x.__float__().$val
    return pyfloat(Math.hypot(xx, yy))
}
...
math.isclose = function(a, b, rel_tol, abs_tol) {
    ...
    if (atol < 0.0 || rtol < 0.0) {
        throw pyValueError('tolerances must be non-negative')
    }
...
math.isfinite = function(x) {
    _checkfloat(x)
    return pybool(isFinite(x.__float__().$val))
}
...
math.isinf = function(x) {
    _checkfloat(x)
    var xx = x.__float__().$val
    return pybool(xx === Infinity || xx === -Infinity)
}
...
math.ldexp = function(x, i) {
    ...
    if (!isinstance(i, [pybool, pyint])) {
        throw pyTypeError('Expected an int as second argument to ldexp.')
    }
    if (xx.$val === 0.0) {
        return xx
    }
...
math.lgamma = function(x) {
    ...
    if (xx <= 0.0) {
        ...
        return pyfloat(Math.log(Math.abs(Math.PI / Math.sin(Math.PI * xx))) - math.lgamma(pyfloat(1 - xx)))
    }

    if (xx < 12.0) {
        var gx = math.gamma(x).$val
        return pyfloat(Math.log(Math.abs(gx)))
    }
...
math.log = function(x, base) {
    ...
    _checkfloat(x)
    if (x.__le__(pyfloat(0.0))) {
        throw pyValueError('math domain error')
    }
    if (x.__eq__(pyfloat(1.0)) && isinstance(base, pyint) && base.$val.gt(1)) {
        return pyfloat(0.0)
    }
    if (typeof base !== 'undefined') {
        _checkfloat(base)
        if (base.__le__(pyfloat(0.0))) {
            throw pyValueError('math domain error')
        }
        var lg_base
        ...
        return pyfloat(math.log2(x).$val / lg_base)
    }
    ...
    if (isinstance(x, pyint)) {
        ...
        if (x.__ge__(pyint.MAX_FLOAT)) {
            return _log2_int(x).__mul__(pyfloat(0.6931471805599453))
        }
    }
    return pyfloat(Math.log(x.__float__().$val))
}
...
var _log2_int = function(x) {
    ...
    var bits = x.$bits()
    if (bits.length < 54) {
        return pyfloat(Math.log2(x.__float__().$val))
    }
    ...
    return pyfloat(Math.log2(num) + exp)
}
...
math.modf = function(x) {
    ...
    return pytuple([pyfloat(frac), pyfloat(int)])
}
...
math.radians = function(x) {
    ...
    return pyfloat(x.__float__().$val * 0.017453292519943295)
}
math.radians.__doc__ = 'radians(x)\n\nConvert angle x from degrees to radians.'

math.sin = function(x) {
    _checkfloat(x)
    return pyfloat(Math.sin(x.__float__().$val))
}
...
math.tan = function(x) {
    _checkfloat(x)
    return pyfloat(Math.tan(x.__float__().$val))
}
math.tan.__doc__ = 'tan(x)\n\nReturn the tangent of x (measured in radians).'

math.tanh = function(x) {
    _checkfloat(x)
    return pyfloat(Math.tanh(x.__float__().$val))
}
math.tanh.__doc__ = 'tanh(x)\n\nReturn the hyperbolic tangent of x.'

math.trunc = function(x) {
    if (x === null) {
        throw pyTypeError("type pyNoneType doesn't define __trunc__ method")
    } else if (!x.__trunc__) {
        throw pyTypeError('type ' + type_name(x) + " doesn't define __trunc__ method")
    }
    return x.__trunc__()
}
