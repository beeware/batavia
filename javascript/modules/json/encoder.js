...
class PyJSONEncoder extends PyObject {
    ...
    __init__(skipkeys = false, ensure_ascii = true, check_circular = true, allow_nan = true,
             sort_keys = false, indent = 0, separators, default_ = pyNone) { // eslint-disable-line indent
        ...
        if (separators === undefined) {
            ...
            this.item_separator = separators.__getitem__(types.pyint(0))
            this.key_separator = separators.__getitem__(types.pyint(1))
        }
    ...
    iterencode(obj) {
        ...
        return types.pylist([this.encode(obj)])
    }
}
...
jstype(PyJSONEncoder, 'JSONEncoder')
    ...
    function encode(obj, indent_level) {
        ...
        if (ret === null) {
            ...
            if (types.isinstance(obj, [types.pylist, types.pytuple])) {
                ...
                ret = encode(default_(obj), indent_level)
            } else {
...
export function dumps(obj, skipkeys = false, ensure_ascii = true, check_circular = true, allow_nan = true,
                      cls = JSONEncoder, indent = 0, separators, default_ = pyNone, // eslint-disable-line indent
                      sort_keys = false, kw) { // eslint-disable-line indent
...
dumps.__doc__ = `Serialize \`\`obj\`\` to a JSON formatted \`\`str\`\`.
    ...
    (\`\`str\`\`, \`\`int\`\`, \`\`float\`\`, \`\`bool\`\`, \`\`pyNone\`\`) will be skipped
    ...
    level of 0 will only insert newlines. \`\`pyNone\`\` is the most compact
    representation.

    If specified, \`\`separators\`\` should be an \`\`(item_separator, key_separator)\`\`
    tuple.  The default is \`\`(', ', ': ')\`\` if *indent* is \`\`pyNone\`\` and
...
export function dump(obj, fp, skipkeys = false, ensure_ascii = true, check_circular = true, allow_nan = true,
                     cls = JSONEncoder, indent = 0, separators, sort_keys = false, default_ = pyNone, kw) { // eslint-disable-line indent
