...
import { pyIndexError, pyOSError, pyOverflowError, pyTypeError, pyValueError } from '../core/exceptions'
...
time.clock = function() {
    return types.pyfloat(new Date().getTime() - time._startTime)
}

time.time = function() {
    // JS operates in milliseconds, Python in seconds, so divide by 1000
    return types.pyfloat(new Date().getTime() / 1000)
}

time.sleep = function(secs) {
    if (secs < 0) {
        throw pyValueError('sleep length must be non-negative')
    }
...
class PyStructTime extends PyObject {
    /*
    ...
    */
    @pyargs({
        ...
        default_args: ['tm_zone', 'tm_gmtoff']
    })
    __init__(tm_year, tm_mon, tm_mday, tm_hour, tm_min, tm_sec, tm_wday, tm_yday, tm_isdst, tm_zone = 'UTC', tm_gmtoff = 0) {
        this.n_fields = 11
...
time.struct_time = jstype(PyStructTime, 'time.struct_time')
...
time.mktime = function(sequence) {
    ...
    if (arguments.length !== 1) {
        throw pyTypeError('mktime() takes exactly one argument (' + arguments.length + ' given)')
    }

    if (!types.isinstance(sequence, [types.pytuple, time.struct_time])) {
        throw pyTypeError('Tuple or struct_time argument required')
    }

    if (sequence.length !== 9) {
        throw pyTypeError('function takes exactly 9 arguments (' + sequence.length + ' given)')
    }

    if (sequence[0] < 1900) {
        // because the earliest possible date is system dependant, use an arbitrary cut off for now.
        throw pyOverflowError('mktime argument out of range')
    }
    ...
    for (var i = 0; i < sequence.length; i++) {
        ...
        if (types.isinstance(item, types.pyfloat)) {
            ...
            throw pyTypeError('an integer is required (got type ' + type_name(item) + ')')
        }
    }
...
time.gmtime = function(seconds) {
    ...
    if (arguments.length > 1) {
        throw pyTypeError('gmtime() takes at most 1 argument (' + arguments.length + ' given)')
    }
    ...
    if (arguments.length === 1) {
        ...
        if (types.isinstance(seconds, [types.pycomplex])) {
            throw pyTypeError("can't convert " + type_name(seconds) + ' to int')
        } else if (!(types.isinstance(seconds, [types.pyint, types.pyfloat, types.pybool]))) {
            throw pyTypeError('an integer is required (got type ' + type_name(seconds) + ')')
        }
        ...
        if (isNaN(date)) {
            ...
            // source: http://ecma-international.org/ecma-262/5.1/#sec-15.9.1.1
            throw pyOSError('Value too large to be stored in data type')
        }
    ...
    return new time.struct_time(types.pytuple(sequence)) // eslint-disable-line new-cap
}
...
time.localtime = function(seconds) {
    ...
    // 0-1 arguments allowed
    if (arguments.length > 1) {
        throw pyTypeError('localtime() takes at most 1 argument (' + arguments.length + ' given)')
    }
    var date
    if (arguments.length === 1) {
        // catching bad types
