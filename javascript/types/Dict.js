...
Dict.prototype.__pos__ = function() {
    throw new TypeError.$pyclass("bad operand type for unary +: 'dict'")
}
...
Dict.prototype.__pow__ = function(other) {
    throw new TypeError.$pyclass("unsupported operand type(s) for ** or pow(): 'dict' and '" + type_name(other) + "'")
}
...
Dict.prototype.__floordiv__ = function(other) {
    ...
    throw new TypeError.$pyclass("unsupported operand type(s) for //: 'dict' and '" + type_name(other) + "'")
}
...
Dict.prototype.__mul__ = function(other) {
    if (types.isinstance(other, [
        ...
        throw new TypeError.$pyclass("can't multiply sequence by non-int of type 'dict'")
    }
}
...
Dict.prototype.__getitem__ = function(key) {
    if (!types.isinstance(key, [
        ...
        types.Bytes])) {
        throw new TypeError.$pyclass("unhashable type: '" + type_name(key) + "'")
    }
    ...
    if (i === null) {
        if (key === null) {
            throw new KeyError.$pyclass('None')
        } else {
            throw new KeyError.$pyclass(key)
        }
    }
...
Dict.prototype.update = function(values) {
    ...
    iter_for_each(updates, function(val) {
        ...
        if (pieces.length !== 2) {
            throw new ValueError.$pyclass('dictionary update sequence element #' + i + ' has length ' + pieces.length + '; 2 is required')
        }
        var key = pieces[0]
        var value = pieces[1]
