...
export function _substitute(format, args) {
    ...
    if (usesKwargs && (!types.isinstance(args[0], types.Dict) || args.length !== 1)) {
        throw new TypeError.$pyclass('format requires a mapping')
    }
    ...
    function Specfier(fullText, index, args) {
        ...
        if (usesKwargs) {
            ...
            if (m === null) {
                throw new ValueError.$pyclass('incomplete format key')
            }
            this.myKey = m[1]
            this.fullText = '%' + m[2]
        } else {
        ...
        this.getNextStep = function(nextChar, currStep) {
            ...
            throw new TypeError.$pyclass('illegal character')
        } // end getNextStep
                ...
                case 3:
                    ...
                    if (char === '*') {
                        ...
                        if (this.usesKwargs) {
                            ...
                            throw new TypeError.$pyclass('* wants int')
                        }
                        ...
                        if (this.fieldWidth.value === '' && this.fieldWidth.numeric === null) {
                            ...
                            if (this.remainingArgs.length === 0) {
                                throw new TypeError.$pyclass('not enough arguments for format string')
                            }
                            this.args.push(arg)
            ...
            function validateType(arg, conversion) {
                ...
                if (/[diouxX]/.test(conversion)) {
                    if (!types.isinstance(arg, [types.Int, types.Float])) {
                        throw new TypeError.$pyclass(`%${conversion} format: a number is required, not str`)
                    }
                    ...
                    if (types.isinstance(arg, types.Str) && arg.valueOf().length > 1) {
                        throw new TypeError.$pyclass('%c requires int or char')
                    } else if (types.isinstance(arg, [types.Int, types.Float])) {
                        if (arg < 0) {
                            throw new OverflowError.$pyclass('%c arg not in range(0xXXXXXXXX)')
                        }
                    }
                } // end outer if
        ...
        while (charIndex < charArray.length && !this.literalPercent) {
            ...
            try {
                ...
                if (err.msg === 'illegal character') {
                    ...
                    throw new ValueError.$pyclass(`unsupported format character '${nextChar}' (0x${charAsHex}) at index ${charIndex + index + 1}`)
                } else {
        ...
        if (this.conversionType === undefined && !this.literalPercent) {
            throw new ValueError.$pyclass('incomplete format')
        }; // end parse main loop
    } // END SPECIFIER
    ...
    if (workingArgs.length !== 0 && !usesKwargs) {
        ...
        workingArgs.remainingArgs.forEach(function(arg) {
            if (!types.isinstance(arg, [types.Bytes, types.Bytearray, types.Dict,
                ...
                throw new TypeError.$pyclass('not all arguments converted during string formatting')
            }
        })
    }
...
export function _new_subsitute(str, args, kwargs) {
    /*
    ...
    */
    ...
    Mode.prototype.checkMode = function(text) {
        ...
        if (this.mode === undefined) {
            ...
            throw new TypeError.$pyclass(`cannot switch from ${this.mode} to ${newMode}`)
        }
    }
    ...
    */
                    ...
                    switch (char) {
                        case ':':
                            ...
                            if (this.conversionFlag === '!') {
                                throw new ValueError.$pyclass('Unknown conversion specifier :')
                            }
                            currentParseGroup = 3
                            break
                        default:
                            if (this.conversionFlag.length === 2) {
                                ...
                                throw new ValueError.$pyclass("expected ':' after conversion specifier")
                            }
                            if (/[!sar]/.test(char)) {
                                // valid the conversion flag
                                this.conversionFlag += char
                            } else {
                                throw new ValueError.$pyclass(`Unknown conversion specifier ${char}`)
                            }
                            break
                    }
                    break
            ...
            switch (currStep) {
                ...
                case 3:
                    ...
                    if (this.fill === undefined) {
                        ...
                        throw new ValueError.$pyclass('Invalid format specifier')
                    }
                case 4:
                    // width
                    if (this.width) {
    ...
    Specifier.prototype.setArg = function() {
        ...
        const parseStack = function(contents) {
            ...
            */
            ...
            if (getItemMatch) {
                if (getItemMatch[1] === '') {
                    throw new ValueError.$pyclass('Empty attribute in format string')
                }
            ...
            if (openBracketRe.test(contents)) {
                throw new ValueError.$pyclass("expected '}' before end of string")
            }
    ...
    Specifier.prototype._convertStr = function() {
        ...
        if (!type.match(/[s ]/)) {
            throw new ValueError.$pyclass(`Unknown format code '${this.type}' for object of type 'str'`)
        }
        ...
        if (this.grouping === ',') {
            if (version.earlier('3.6')) {
                throw new ValueError.$pyclass("Cannot specify ',' with 's'.")
            } else {
                throw new ValueError.$pyclass("Cannot specify ',' or '_' with 's'.")
            }
        }

        if (this.sign) {
            throw new ValueError.$pyclass('Sign not allowed in string format specifier')
        }

        if (this.alternate) {
            throw new ValueError.$pyclass('Alternate form (#) not allowed in string format specifier')
        }

        if ((this.align === '=') || (this.fill === '0')) {
            throw new ValueError.$pyclass("'=' alignment not allowed in string format specifier")
        }
    ...
    Specifier.prototype._convertNumber = function() {
        ...
        if (this.type === 'c' && this.sign) {
            throw new ValueError.$pyclass("Sign not allowed with integer format specifier 'c'")
        }

        if (this.grouping && !type.match(/[deEfFgG%]/)) {
            // used a , with a bad conversion type:
            throw new ValueError.$pyclass(`Cannot specify ',' with '${type}'.`)
        }

        if (this.precision && type.match(/[bcdoxXn]/)) {
            throw new ValueError.$pyclass('Precision not allowed in integer format specifier')
        }

        if (type === 's') {
            throw new ValueError.$pyclass("Unknown format code 's' for object of type 'int'")
        }
