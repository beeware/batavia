...
export default function next(args, kwargs) {
    ...
    if (kwargs && Object.keys(kwargs).length > 0) {
        throw new TypeError.$pyclass('next() takes no keyword arguments')
    }
    if (!args || args.length === 0) {
        throw new TypeError.$pyclass('next expected at least 1 arguments, got 0')
    }
    if (args.length > 2) {
        throw new TypeError.$pyclass('next expected at most 2 arguments, got ' + args.length)
    }
    ...
    try {
        ...
        if (e instanceof StopIteration.$pyclass) {
            ...
            throw new TypeError.$pyclass("'" + type_name(args[0]) + "' object is not an iterator")
        }
    }
}
