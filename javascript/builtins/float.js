...
export default function float(args, kwargs) {
    if (args.length > 1) {
        throw new TypeError.$pyclass('float() takes at most 1 argument (' + args.length + ' given)')
    }
    if (args.length === 0) {
    ...
    if (types.isinstance(value, types.Str)) {
        if (value.length === 0) {
            throw new ValueError.$pyclass('could not convert string to float: ')
            ...
            throw new ValueError.$pyclass("could not convert string to float: '" + args[0] + "'")
        }
