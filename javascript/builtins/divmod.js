...
export default function divmod(args, kwargs) {
    ...
    if (kwargs && Object.keys(kwargs).length > 0) {
        throw new TypeError("divmod() doesn't accept keyword arguments")
    }
    if (!args || args.length !== 2) {
        throw new TypeError('divmod expected 2 arguments, got ' + args.length)
    }
    ...
    if (types.isinstance(args[0], notAllowedTypes) || types.isinstance(args[1], notAllowedTypes)) {
        throw new TypeError("unsupported operand type(s) for divmod(): '" + type_name(args[0]) + "' and '" + type_name(args[1]) + "'")
    }
