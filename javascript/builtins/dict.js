...
export default function dict(args, kwargs) {
    ...
    if (args.length > 1) {
        throw new TypeError.$pyclass('dict expected at most 1 arguments, got ' + args.length)
    }
    ...
    if (types.isinstance(args[0], types.Bytearray) || (types.isinstance(args[0], types.Bytes) && args[0].val.length > 0) || (types.isinstance(args[0], types.Range) && args[0].length > 0) || (types.isinstance(args[0], types.FrozenSet) && args[0].data.size > 0)) {
        throw new TypeError.$pyclass('cannot convert dictionary update sequence element #0 to a sequence')
    }
    var i
        ...
        for (i = 0; i < args[0].length; i++) {
            if (args[0][i].length !== 2) {
                ...
                if (types.isinstance(args[0][i], [types.Bool, types.Int])) {
                    throw new TypeError.$pyclass('cannot convert dictionary update sequence element #' + i + ' to a sequence')
                } else {
                    throw new ValueError.$pyclass('dictionary update sequence element #' + i + ' has length ' + args[0][i].length + '; 2 is required')
                }
            }
        }
    }
